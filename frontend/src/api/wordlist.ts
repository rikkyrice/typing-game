...
import { WordListSummaryArray, WordListSummary, WordListArray, WordList } from '@/models/wordlist';

function getWordLists() {
  return APIClient.getAPI<WordListSummaryArray>(API_PATHS.WORDLISTS.GET)
    .then((data) => Promise.resolve(data))
    .catch((err) => Promise.reject(err));
}

function getWordList(wordListId: string) {
  return APIClient.getAPI<WordListSummary>(API_PATHS.WORDLIST.GET(wordListId))
    .then((data) => Promise.resolve(data))
    .catch((err) => Promise.reject(err))
}
